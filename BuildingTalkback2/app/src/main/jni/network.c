...
int wait_for_client()
{
	...
	if (server_sockfd < 0)
	{
		LOGD("cannot create server_sockfd: %d", server_sockfd);
		return -1;
	}
	...
	{
		LOGD("setsockopt error: %d, %s", errno, strerror(errno));
		return -1;
	}

	if (bind(server_sockfd, (struct sockaddr *) &server_addr, server_len))
	{
		LOGD("bind socket error: %d, %s", errno, strerror(errno));
		return -1;
	}
...
int start_client()
{
	...
	LOGD("connecting server, IP is %s", server_ip);

	sockfd = socket(AF_INET, SOCK_STREAM, 0);
	if (sockfd < 0)
	{
		LOGD("cannot create sockfd: %d", sockfd);
		return -1;
	}

	if (connect(sockfd, (struct sockaddr *) &server_addr, server_len))
	{
		LOGD("cannot connect to server, errno = %d, error = %s", errno, strerror(errno));
		return -1;
	}
