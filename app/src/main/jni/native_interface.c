...
JNIEXPORT jint JNICALL JNI_OnLoad(JavaVM* vm, void* reserved)
{
	...
	mid_on_reg_done			= (*env)->GetStaticMethodID(env, native_interface, "onRegistrationDone", "(Ljava/lang/String;)V");
	mid_on_reg_failed		= (*env)->GetStaticMethodID(env, native_interface, "onRegistrationFailed", "(Ljava/lang/String;)V");
	...
	mid_on_call_ringing		= (*env)->GetStaticMethodID
			(env, native_interface, "onCallRinging", "(Ljava/lang/String;Z)V");
	mid_on_call_established = (*env)->GetStaticMethodID
			(env, native_interface, "onCallEstablished", "(Ljava/lang/String;Z)V");
...
JNIEXPORT void JNICALL Java_cn_edu_hust_buildingtalkback_jni_NativeInterface_sendVideo
		(JNIEnv *env, jclass clazz, jbyteArray data, jint num)
{
	LOGD("BT_VIDEOS -- Begin NativeSend");
	uint8_t buf[num];
	(*env)->GetByteArrayRegion(env, data, 0, num, (jbyte *)buf);
	LOGD("BT_VIDEOS -- GEt Byte Success");
	if(buf == NULL)
		LOGE("BT_VIDEOS -- buf == NULL");
	LOGD("BT_VIDEOS -- Begin capture_send_buf");

	sendVideoFrame(buf, num);
	LOGD("BT_VIDEOS -- One capture_sended");
}
...
// 初始化SIP环境
JNIEXPORT jint JNICALL Java_cn_edu_hust_buildingtalkback_jni_NativeInterface_sipInit(JNIEnv *env, jclass clazz) {
	return sip_init();
}
...
void on_reg_done(JNIEnv *env, const char *target)
{
	jstring str = (*env)->NewStringUTF(env, target);
	(*env)->CallStaticVoidMethod(env, native_interface, mid_on_reg_done, str);
	(*env)->DeleteLocalRef(env, str);
}

void on_reg_failed(JNIEnv *env, const char *target)
{
	jstring str = (*env)->NewStringUTF(env, target);
	(*env)->CallStaticVoidMethod(env, native_interface, mid_on_reg_failed, str);
	(*env)->DeleteLocalRef(env, str);
}
