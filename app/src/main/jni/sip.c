...
void *event_handler_entry(void *arg)
{
	...
	while (sip_on)
	{
		...
		if (event == NULL)
			continue;
		LOGD("event->rid = %2d, cur_rid = %2d, event->type = %2d, info = %s", event->rid, cur_rid, event->type, event->textinfo);
		switch (event->type)
		{
			case EXOSIP_REGISTRATION_SUCCESS:   // event_type = 1
				if (event->rid >= cur_rid) {
					cur_rid = event->rid;
					on_reg_done(env, srv);
				}
				break;
			...
			case EXOSIP_REGISTRATION_FAILURE:   //  event_type = 2
				//			LOGD("JNI LOG=== SIP REG FAIL event->code = %3d, info = %s", event->response->status_code, event->textinfo);
				//			if (event->response->status_code != 401 && event->response->status_code != 407)
				if (event->rid >= cur_rid) {
					cur_rid = event->rid;
					LOGD("JNI LOG=== SIP REG FAIL");
					on_reg_failed(env, srv);
				}
				break;
			...
			case EXOSIP_CALL_INVITE:            // event_tpye = 5
				...
				if (cid == -1)
				{
					...
					LOGD("on_call_ringing from %s", event->response->from->url->username);
			...
			case EXOSIP_CALL_ANSWERED:          // event_type = 10
				...
				LOGD("JNI LOG=== SIP on_call_established, username = %s", event->response->to->url->username);
				on_call_established(env, event->response->to->url->username, 1);
				break;


			case EXOSIP_CALL_REQUESTFAILURE:    // event_type = 12
				LOGD("EXOSIP_CALL_REQUESTFAILURE, status_code = %d, cid=%d", event->response->status_code, event->cid);
			...
			case EXOSIP_CALL_ACK:               // event_type = 15
				...
				LOGD("JNI LOG=== SIP on_call_established, username = %s", event->response->from->url->username);
	...
	LOGD("JNI LOG=== SIP detach_thread");
	detach_thread();
...
int sip_init()
{
	...
	if (ctx == NULL)
	{
		LOGD("JNI LOG=== SIP eXosip_malloc() failed");
		return -1;
	}
	...
	if (res)
	{
		LOGD("JNI LOG=== SIP eXosip_init() failed, %d", res);
		return -1;
	}
	...
	if (res)
	{
		LOGD("JNI LOG=== SIP eXosip_listen_addr() failed, %d", res);
//		eXosip_quit(ctx);
		return -1;
	}
	...
	if (res != 0)
	{
		LOGD("JNI LOG=== SIP pthread_create() failed, %d", res);
//		eXosip_quit(ctx);
		return -1;
	}

	LOGD("JNI LOG=== SIP %s done", __func__);
...
//		return -1;
	...
	LOGD("JNI LOG=== SIP begin to register: server ip = %s, username = %s, password = %s", srv, usr, psw);

	reg_id = eXosip_register_build_initial_register(ctx, sip_usr_srv, sip_srv, NULL, 1800, &reg);
	if (reg_id < 0)
	{
		UNLOCK();
		LOGD("JNI LOG=== SIP eXosip_register_build_initial_register() failed, %d", reg_id);
		return -1;
	}
	...
	{
		UNLOCK();
		LOGD("JNI LOG=== SIP eXosip_register_send_register() failed, %d", res);
		return -1;
	}
...
int sip_make_call(const char *user)
{
	...
	if (res != 0)
	{
		LOGD("JNI LOG=== SIP eXosip_call_build_initial_invite() failed, %d", res);
		UNLOCK();
		return -1;
	}

	{
		...
		char local_ip[128];
		eXosip_guess_localip(ctx, AF_INET, local_ip, 128);
		LOGD("JNI LOG=== SIP guess_localip = %s", local_ip);
		snprintf(tmp, 4096,
				 "o=%s@%s 0 0 IN IP4 %s\r\n"
						 "s=Session SIP/SDP\r\n"
						 "c=IN IP4 %s\r\n"
	...
	LOGD("JNI LOG=== SIP Call the cid = %d", cid);
	UNLOCK();
